...
class SyntheticEventGenerator:
    ...
    def load_data(self):
        df_bank = pd.read_parquet(self.clean_bank_path)
    ...
    def simulate_mab_environment(self):
        df_bank, offers = self.load_data()
        ...
        for index, client in df_bank.iterrows():
            ...
            impression_time = base_date + timedelta(
                days=np.random.randint(0, 30), hours=np.random.randint(0, 23)
            )
        ...
        df_events.to_parquet(
            "data/kaggle/synthetic_enrichment/offer_events.parquet", index=False
        )
        df_rewards.to_parquet(
            "data/kaggle/synthetic_enrichment/delayed_rewards.parquet", index=False
        )

        df_events.to_csv("data/kaggle/synthetic_enrichment/offer_events.csv", index=False)
        df_rewards.to_csv(
            "data/kaggle/synthetic_enrichment/delayed_rewards.csv", index=False
        )

        print(f"Gerados {len(df_events)} eventos de impressão.")
        print(
            f"Geradas {len(df_rewards[df_rewards['reward_value'] > 0])} conversões com sucesso."
        )
        print("Arquivos Parquet e CSV salvos em data/kaggle/synthetic_enrichment/")

    def generate_events(self):
        # Certifique-se de que as pastas existam antes de rodar
        import os

        os.makedirs("data/kaggle/synthetic_enrichment", exist_ok=True)
        self.simulate_mab_environment()
