...
2. Roda simulação para Baseline, Thompson e UCB1 (seed 42).
...
from bandit.simulator import run_simulation, DEFAULT_BANK_PATH

OUT_DIR = Path("data/experiments/etapa3")
SEED = 42
...
def main():
  ...
  pd.DataFrame(all_history).to_parquet(OUT_DIR / "metrics_timeseries.parquet", index=False)
