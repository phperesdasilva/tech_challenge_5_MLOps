...
class ThompsonSamplingPolicy(Policy):
  def __init__(self, arm_ids: list[str], alpha0: float = 1.0, beta0: float = 1.0):
  ...
  def select_arm(self, eligible_arm_ids: list[str]) -> str:
    samples = {
      a: self.rng.beta(self.alpha[a], self.beta[a])
...
class UCB1Policy(Policy):
  ...
  def __init__(self, arm_ids: list[str], c: float = 2.0):
    self.c = c
