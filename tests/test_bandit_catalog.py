...
def test_load_catalog_reads_offers_from_json(tmp_path):
    ...
    catalog_path.write_text(json.dumps({"ofertas": [{"id": "A"}]}), encoding="utf-8")
...
def test_load_catalog_returns_list_of_offers_when_file_exists(tmp_path):
    catalog_path = tmp_path / "catalog.json"
    catalog_path.write_text(json.dumps({"ofertas": [{"id": "B"}, {"id": "C"}]}), encoding="utf-8")
...
def test_is_eligible_respects_min_age_and_housing_rule():
    ...
    rules = {"idade_minima": 18, "requer_emprestimo_habitacional": True}
...
def test_get_eligible_offers_filters_offers_by_rules():
    ...
    offers = [
        ...
        {"id_braco": "1", "regras_elegibilidade": {"idade_minima": 30, "requer_emprestimo_habitacional": True}},
    ]

    eligible = get_eligible_offers(client, offers)

    assert [offer["id_braco"] for offer in eligible] == ["0"]


def test_expected_reward_returns_prior_times_reward_value():
    offer = {"prior_conversao_sintetica": 0.4, "valor_recompensa": 20}
